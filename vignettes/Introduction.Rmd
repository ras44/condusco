...
Arnold and Barb put their heads together and think "Wouldn't it be nice if we could each define our own configuration and then dynamically generate the queries when we run Whizbang?".
...
Whizbang 2.0 is successfully deployed and starts making waves around the globe.  Orders for VT-100s rise.  Euphoric with the success of Whizbang 2.0, Arnold's boss calls a meeting.
...
Arnold and Barb put their heads together and think "Wouldn't it be nice if we could get region codes dynamically via a database query and then dynamically run Whizbang for each region on a particular date?".

### Whizbang 3.0: Query-driven function calls
Barb finds the [condusco R package](https://github.com/ras44/condusco).  With condusco, she creates a pipeline called `create_daily_region_sample` and repeatedly runs it for each of the regions she gets from a query.
...
The 'run_pipeline' function runs the query provided to it, called the **invocation query**, and receives a result.  For each row in the result, it calls the pipeline and passes that row to the pipeline as a dataframe.
